     ...
     char *argv[])			/* I - Command-line arguments */
{
  ...
  */
  ...
  for (i = 1; i < argc; i ++)
  {
    ...
    else if (!strcmp(argv[i], "--no-website"))
    {
      Website = 0;
    }
	...
	*/
    ...
    case HTTP_STATE_HEAD :
	  ...
	  return (respond_http(client, HTTP_STATUS_OK, NULL, "text/strings", 0));
        else if (Website && (!strcmp(client->uri, "/icon.png") || !strcmp(client->uri, "/icon-lg.png") || !strcmp(client->uri, "/icon-sm.png")))
	  return (respond_http(client, HTTP_STATUS_OK, NULL, "image/png", 0));
	else if (Website && (!strcmp(client->uri, "/") || !strcmp(client->uri, "/media") || !strcmp(client->uri, "/supplies")))
	...
	else
	{
	  ...
	  */
	  ...
	  else if (Website && !strcmp(client->uri, "/media"))
	  {
	   /*
	    * Show web media page...
	    */
	  ...
	  else if (Website && !strcmp(client->uri, "/supplies"))
	  {
	   /*
...
process_job(ippeve_job_t *job)		/* I - Job */
{
  ...
  if (job->printer->command)
  {
    ...
    */
    ...
    if (job->client)
    {
      snprintf(val, sizeof(val), "CLIENT_HOSTNAME=%s", job->client->hostname);
      myenvp[myenvc ++] = strdup(val);
    }
...
usage(int status)			/* O - Exit status */
{
  ...
  _cupsLangPuts(stdout, _("--no-website            Disable website"));
